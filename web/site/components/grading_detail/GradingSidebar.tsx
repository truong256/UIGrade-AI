    ...
    return (
        <aside className="lg:sticky lg:top-20 lg:col-span-4 lg:self-start">
            <section className="rounded-3xl border border-sky-100 bg-white p-4 shadow-sm">
                ...
                <p className="mt-0.5 text-xs text-sky-700 font-medium">Hạn nộp: {formatDateTime(assignment?.dueAt)}</p>
                ...
                <div className="mt-4 flex border-b border-sky-100">
                    <button
                        ...
                        className={`flex-1 border-b-2 pb-2 text-xs font-bold uppercase tracking-wider transition ${
                            tab === "list" ? "border-sky-600 text-sky-700" : "border-transparent text-slate-400 hover:text-slate-600"
                        }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 border-b-2 pb-2 text-xs font-bold uppercase tracking-wider transition ${
                            tab === "config" ? "border-sky-600 text-sky-700" : "border-transparent text-slate-400 hover:text-slate-600"
                        }`}
                    >
                        Cấu hình
                    </button>
                    ...
                    <>
                        <div className="relative mt-3">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-slate-200 bg-slate-50/50 py-2 pl-9 pr-3 text-xs outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
                            />
                        </div>

                        <div className="mt-3 flex max-h-[760px] flex-col gap-1.5 overflow-y-auto pr-1">
                            {loading ? (
                                <div className="rounded-2xl border border-dashed border-sky-100 bg-sky-50/30 px-4 py-8 text-center text-xs text-slate-500">
                                    Đang tải danh sách sinh viên...
                                </div>
                            ) : visibleStudents.length === 0 ? (
                                <div className="rounded-2xl border border-dashed border-sky-100 bg-sky-50/30 px-4 py-8 text-center text-xs text-slate-400">

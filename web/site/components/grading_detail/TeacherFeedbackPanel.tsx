    ...
    return (
        <section className="rounded-3xl border border-sky-100 bg-white p-5 shadow-sm">
            <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <span className="material-symbols-outlined text-sky-600 text-[20px]">rate_review</span>
                Phản hồi & Nhận xét của Giảng viên
            </h3>

            <div className="mt-3 rounded-2xl bg-sky-50/40 border border-sky-100 p-3.5 text-xs text-slate-600 space-y-2">
                <p>
                    <span className="font-semibold text-slate-900">Mã nguồn / Link file:</span>{" "}
                    {detail?.repositoryUrl || detail?.fileUrl ? (
                        <a href={detail.repositoryUrl || detail.fileUrl} target="_blank" rel="noreferrer" className="text-sky-700 font-medium hover:underline">
                            {detail.repositoryUrl || detail.fileUrl}
                        </a>
                    ) : (
                        "Không có"
            ...
            <textarea
                ...
                className="mt-3 min-h-[140px] w-full rounded-2xl border border-slate-200 p-3.5 text-xs outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"

...
export function SubmissionHistory({ items }: Props) {
    ...
    return (
        <section className="pb-6">
            ...
            <div className="overflow-hidden rounded-3xl border border-sky-100 bg-white shadow-xs">
                {!items.length ? (
                    ...
                    <div className={shouldScroll ? "max-h-[360px] overflow-y-auto" : ""}>
                        <div className="divide-y divide-slate-100">
                            {items.map((item) => (
                                <div
                                    key={item._id}
                                    className="flex items-center justify-between gap-4 p-4 transition hover:bg-sky-50/30"
                                >
                                    ...
                                    <div className="shrink-0 text-right">
                                        ...
                                        <p className="text-xs font-bold text-sky-950">{item.score}</p>
                                    </div>
                                </div>
                            ))}
                        </div>

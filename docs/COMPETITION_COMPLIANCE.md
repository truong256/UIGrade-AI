...
- **Bản phát hành chính thức hiện tại:** [v1.0.0](https://github.com/truong256/UIGrade-AI/releases/tag/v1.0.0)
- **Git Tag tương ứng:** `v1.0.0`
...
- **Trạng thái:** **PASS** (Đạt điểm tối đa)
- **Bằng chứng:**
  - Đã gắn Git Tag phiên bản chính thức: `v1.0.0`.
  - Công bố bản phát hành trên GitHub Releases: [UIGrade AI v1.0.0 — Competition Release](https://github.com/truong256/UIGrade-AI/releases/tag/v1.0.0).
...
- **Bằng chứng:**
  ...
  - Toàn bộ mã nguồn bên ngoài (như dữ liệu tham khảo từ paper) được cách ly trong thư mục riêng `web/project-files/` và được dẫn nguồn rõ ràng trong `NOTICE.md`.
...
- **Bằng chứng:**
  ...
  - **Hệ thống theo dõi lỗi (Bug Tracker) thực tế:**
    - Sử dụng **GitHub Issues** với các mẫu chuẩn YAML trong `.github/ISSUE_TEMPLATE/` (`bug_report.yml`, `feature_request.yml`, `config.yml`).
    - Quy trình xử lý lỗi thực tế được lưu vết rõ ràng: tạo Issue, đính kèm commit giải quyết (`Fixes #X`), chạy kiểm thử xác nhận và đóng Issue.
  ...
  - **Tài liệu kiến trúc chuyên sâu:**
    - [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md): Sơ đồ kiến trúc tổng thể, phân tầng hệ thống, cơ chế bảo mật sandbox giải nén và phân quyền dữ liệu RLS.
    - [docs/AI_ARCHITECTURE.md](docs/AI_ARCHITECTURE.md): Kiến trúc AI v2.0 Grader-Critic, cơ chế neo bằng chứng (Evidence Grounding), nguyên tắc con người kiểm soát (Human-in-the-Loop) và kết quả thực nghiệm mô hình.
